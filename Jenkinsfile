  ...
  stage('Test') {
   steps {
     ...
     post {
       	...
       	failure {
       	   script{
		   def response = httpRequest authentication: 'jenkins-creds',acceptType: 'TEXT_PLAIN', ignoreSslErrors: true, url: 'http://34.248.134.77:8080/job/jetpetstore-pipleine/'+"${BUILD_NUMBER}"+'/consoleText';
		   println '+++++++++++++++++++++++++++++++++++++++++++++\n'+response.content;
