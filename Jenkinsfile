  ...
  stage('Test') {
   steps {
     ...
     post {
       	...
       	failure {
       	   script{
		   def response = httpRequest authentication: 'jenkins-creds', customHeaders: [[maskValue: false, name: 'Accept', value: 'text/html,application/xhtml+xml,application/xml;q=0.9,image/webp,image/apng,*/*;q=0.8']], ignoreSslErrors: true, url: 'http://34.248.134.77:8080/job/jetpetstore-pipleine/'+"${BUILD_NUMBER}"+'/consoleText';
		   println '+++++++++++++++++++++++++++++++++++++++++++++\n'+response.content;
